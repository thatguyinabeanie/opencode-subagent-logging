...
export const sessionHierarchy = new Map<string, { parentId: string | null; depth: number, agentType?: string }>();
const subagentStats = new Map<string, { startTime: number; toolCount: number; errorCount: number }>();
...
function isFileContentTool(toolName: string, input: any): boolean {
  ...
  const dataFileExtensions = [
    ...
    '.parquet', '.avro', '.orc', '.jsonl', '.ndjson', '.txt', '.dat',
...
export function formatEvent(event: any): { message: string | null; level: 'INFO' | 'DEBUG'; sessionId: string | null } {
  ...
  if (type === 'session.idle') {
    ...
    const tokenSummary = tokens ? ` (Total Tokens: Input: ${tokens.input}, Output: ${tokens.output})` : '';
      ...
      case 'running': {
        ...
        if (toolName === 'task') {
          ...
          if (!loggedSubagentStarts.has(subagentSessionId)) {
            ...
            sessionHierarchy.set(subagentSessionId, { parentId: sessionId, depth: parentInfo.depth + 1, agentType });
            subagentStats.set(subagentSessionId, { startTime: Date.now(), toolCount: 0, errorCount: 0 });
          ...
          const details = state.input?.description || state.input?.filePath || state.input?.command || state.input?.pattern || toolName;
          const truncatedDetails = details && typeof details === 'string' ? (details.length > 80 ? details.slice(0, 80) + '...' : details) : toolName;
